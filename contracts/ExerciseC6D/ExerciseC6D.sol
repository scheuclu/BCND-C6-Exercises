...
contract ExerciseC6D {
    ...
    uint numResponseInfos;
    mapping (uint => ResponseInfo) responseInfos;
    function createResponseInfo(
            address requester,
            bool isOpen) public returns (uint) {   
        uint newInt = numResponseInfos++;
    ...
    function registerOracle() external payable {
        ...
        require(
            ...
            "Registration fee no met in call to registerOracle()."
        );
    ...
    function submitOracleResponse(
        ...
        responseInfos[oracleResponses[key]].responses[statusId].push(msg.sender);
